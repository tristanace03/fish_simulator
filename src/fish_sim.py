...
CONFIG = {
    ...
    "fish_speed": 120.0,
    "predator_speed": 145.0,   # slightly faster than fish
    "predator_turn_rate": 5.0, # radians/sec, limits how sharply predator can turn

    "fish_turn_rate": 6.0,     # radians/sec, how fast fish can turn

    "eat_radius": 12.0,        # predator eats fish if within this distance
    ...
    "input_dim": 8,
    "hidden_dim": 12,
    "output_dim": 1,           # turn only (keep speed constant initially)
...
class World:
    ...
    def step(self, fish_nets: List[TinyNet], dt: float) -> None:
        ...
        for idx, (f, net) in enumerate(zip(self.fish, fish_nets)):
            ...
            # Move
            f.x += math.cos(f.heading) * self.fish_speed * dt
            f.y += math.sin(f.heading) * self.fish_speed * dt
